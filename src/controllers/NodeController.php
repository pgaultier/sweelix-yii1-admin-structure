...
class NodeController extends Controller
{
    ...
    {
        if (isset($this->breadCrumb[$currentNodeId]) === false) {
            ...
            if ($currentNode !== null) {
                ...
                foreach ($path as $element) {
                    $this->breadCrumb[$currentNodeId][] = array(
        ...
        return $this->breadCrumb[$currentNodeId];
    }
        ...
        try {
            ...
            if (isset($_POST[Html::modelName('sweelix\yii1\ext\entities\Group')]) === true) {
                ...
                for ($i = 0; $i < count($groups); $i++) {
                    $selectedTags = NodeTag::model()->findAllByAttributes(array(
    ...
    public function actionMoveContent($contentId, $page = 0)
    {
        try {
            ...
            if ($content !== null) {
                ...
                if (($content !== null)
                    && (in_array($target, array('before', 'after')) === true)
                    && ($targetId !== false)
                ) {
     ...
     * @since  1.2.0
     */
    private function renderListContent($page = 0)
    {
        try {
            Yii::trace(__METHOD__ . '()', 'sweelix.yii1.admin.structure.controllers');
