...
class ContentController extends Controller
{
    ...
    {
        if (isset($this->breadCrumb[$currentContentId]) === false) {
            ...
            if ($currentContent !== null) {
                if ($currentContent->node !== null) {
                    ...
                    foreach ($path as $element) {
                        $this->breadCrumb[$currentContentId][] = array(
        ...
        return $this->breadCrumb[$currentContentId];
    }
        ...
        $mainMenu = array(
            'main' => array(
                array(
                    ...
                    'url' => array(
                        'node/',
                        'nodeId' => ($this->currentNode !== null) ? $this->currentNode->nodeId : ''
                    ),
                    'active' => false,
                ),
                array(
        ...
        try {
            ...
            if (isset($_POST[Html::modelName('sweelix\yii1\ext\entities\Group')]) === true) {
                ...
                for ($i = 0; $i < count($groups); $i++) {
                    $selectedTags = ContentTag::model()->findAllByAttributes(array(
                ...
                foreach ($_POST[Html::modelName('sweelix\yii1\admin\core\models\Meta')] as $swfMeta) {
                    ...
                    if ($formMeta->validate() === true) {
                        if (CPropertyValue::ensureBoolean($formMeta->metaModeOverride) === true) {
                            $contentMeta = ContentMeta::model()->findByPk(array(
                                'contentId' => $formMeta->contentId,
