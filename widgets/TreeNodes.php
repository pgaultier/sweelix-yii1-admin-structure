...
{
    /**
     * @var array collection list of nodes
     */
    private $nodes = null;
    ...
    public function run()
    {
        ...
        $this->nodes = Helper::linearizeNodesToArray(Node::model()->findAll(array('order' => 'nodeLeftId')));
        echo $this->convertNodesToHtml($this->nodes, true);
    ...
    public function convertNodesToHtml($data, $start = false)
    {
        ...
        foreach ($data as $row) {
            $linkClass = '';
            if (($this->node !== null)
                && ($row['node']->nodeLeftId <= $this->node->nodeLeftId)
                && ($this->node->nodeRightId <= $row['node']->nodeRightId)
            ) {
                $linkClass = 'path';
            }
            $str .= Html::tag(
                ...
                array(
                    ...
                    'data-mode' => 'replace',
                    'data-url-move' => Html::normalizeUrl(array(
                        'node/move',
                        'nodeId' => ($this->node !== null) ? $this->node->nodeId : ''
                    )),
                ),
                Html::link(
                    $row['node']->nodeTitle,
